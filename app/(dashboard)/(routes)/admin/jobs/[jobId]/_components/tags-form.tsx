...
const TagsForm = ({ initialData, jobId }: TagsFormProps) => {
  ...
  const handlePromptGeneration = async () => {
    try {
      setIsPrompting(true);
      const customPrompt = `Generate an array of top 10 keywords related to the job proffesion${prompt}. These keywords should encompass various aspects of the profession, including skills, responsibilities, tools, and technologies commonly associated with it. Aim for a diverse set of keyword that accurately represent the breadth of the proffesion. Your output should be a list/array of keywords. Just return me the array alone`;
      ...
      {isEditing && (
        ...
        <div className="flex items-center gap-2 justify-end mt-4">
          ...
          disabled={isSubmitting}
           onClick={() => onSubmit({tags: jobTags})}>Save</Button>
        </div>
        </>
      )}
    </div>
